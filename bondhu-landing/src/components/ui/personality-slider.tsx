...
export function PersonalitySlider({ 
  ...
  return (
    <div className={cn("space-y-3 sm:space-y-4", className)}>
      ...
      <div className="sm:hidden space-y-2">
        {sliderLabels.map((item) => (
          <motion.button
            ...
            tabIndex={0}
            className={cn(
              "relative w-full py-3 px-4 rounded-xl border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/50",
              "flex items-center justify-between",
              getSliderColor(item.value),
              value === item.value && "ring-2 ring-primary shadow-md",
          ...
          <div className="flex justify-between items-center space-x-2 sm:space-x-3">
            {sliderLabels.map((item) => (
              <motion.button
                ...
                tabIndex={0}
                className={cn(
                  "relative w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:ring-offset-2",
      ...
      {/* Current selection display - Desktop only */}
      <AnimatePresence>
        {(value || hoveredValue) && (
          <motion.div
